...
private:
    // 对指定车牌颜色进行匹配
    vector<NYPlate> colorMatch(Mat src, Color r);
    
    // 获取指定颜色的车牌区域灰度图
    Mat convertColorToGray(Mat src, Color r);
    ...
    // 将指定区域调整为标准尺寸
    Mat resizeToNormalSize(Mat src, Size rect_size, Point2f center);
    
    // 将Mat调整为标准尺寸
    Mat resizeToNormalSize(Mat src);
    
    // 将指定区域调整为标准角度
    bool rotateToNormalAngle(Mat in, Mat &out, RotatedRect roi_rect);
